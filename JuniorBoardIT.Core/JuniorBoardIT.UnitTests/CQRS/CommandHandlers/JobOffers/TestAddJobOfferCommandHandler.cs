...
{
    [TestFixture]
    public class TestAddJobOfferCommandHandler
    {
        ...
        public void SetUp()
        {
            ...
            user.Setup(x => x.UGID).Returns("29dd879c-ee2f-11db-8314-0800200c9a66");
        }

        [Test]
        public void TestAddJobOfferCommandHandler_ModelIsValid_ShouldAdd_NewEntity()
        {
            // Arrange
            var model = jobOfferViewModel();

            var command = new AddJobOfferCommand() { Model = model };
            var handler = new AddJobOfferCommandHandler(context.Object, user.Object);

            // Act
            handler.Handle(command);

            // Assert
            ClassicAssert.AreEqual(model.JOCGID, jobOffers[0].JOCGID);
